"""File-handling ops: mv, cp, ln, have similar filename arguments:
- Zero or more SOURCE_FILENAMEs, where each SOURCE_FILENAME is a filename or glob.
- Exactly one TARGET_FILENAME (also a filename or glob).
        ...
        if len(sources) == 0:
            ...
            if target_path.exists():
                if target_path.is_file():
                    if len(self.roots) > 1:
                        raise marcel.exception.KillCommandException('Cannot use multiple sources with a file target')
            else:
                if len(self.roots) > 1:
                    raise marcel.exception.KillCommandException('Cannot use multiple sources with a non-existent target')
